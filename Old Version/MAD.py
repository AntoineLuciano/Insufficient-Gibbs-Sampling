...
def MAD_init(N, med, MAD, distribution):
    ...
    if distribution in ["lognormal","weibull"]:
        ...
        n = N // 2
        k = np.ceil(n / 2)
        if distribution == "lognormal":init_theta = [np.log(med), MAD / med,None]
        else: init_theta = [0,med/np.log(2),shape]
        if N % 2 == 0:
            return np.repeat(
                [
                    med-.01*MAD,
    ...
    if distribution == "normal":
        Z = np.random.normal(loc, scale, N)
    elif distribution == "cauchy":
        Z =scipy.stats.cauchy(loc=loc, scale=scale).rvs(N)
    elif distribution == "weibull" or distribution == "translated_weibull":
        Z =scipy.stats.weibull_min(c=shape, loc=loc, scale=scale).rvs(N)
    else : raise Exception("ERROR: distribution {} not implemented !".format(distribution))
    m_Z, s_Z = np.median(Z), scipy.stats.median_abs_deviation(Z)
    ...
    return X_0, np.round(init_theta,8)
    ...
    if med in xij and xmad in xij:
        ...
        if type(zone_odd(xi, med, MAD)) != str and type(zone_odd(xj, med, MAD)) != str:
            if sorted([zone_odd(xi, med, MAD), zone_odd(xj, med, MAD)]) in [
                ...
                case = "4a"
                if distribution == "normal":
                    xnew1 = np.random.normal(loc, scale, 1)[0]
                elif distribution == "cauchy":
                    xnew1 = scipy.stats.cauchy(loc, scale).rvs(1)[0]
                elif distribution == "weibull":
                    xnew1 = scipy.stats.weibull_min(c=shape, scale=scale, loc=loc).rvs(
                        1
                    )[0]
                elif distribution == "translated_weibull":
                    xnew1 = scipy.stats.weibull_min(c=shape, scale=scale).rvs(1)[0]
...
def Gibbs_med_MAD(
    ...
    Args:
        ...
        prior_scale (str): Prior distribution of the scale parameter ("gamma", "cauchy", "uniform" or "none").
        prior_shape (str): Prior distribution of the shape parameter ("normal", "cauchy", "uniform" or "none").
        par_prior_loc (list, optional): Prior hyperparameters for the location parameter. Defaults to [0, 1].
        par_prior_scale (list, optional): Prior hyperparameters for the location parameter. Defaults to [1, 1].
        par_prior_shape (list, optional): Prior hyperparameters for the location parameter. Defaults to [0, 1].
        std_prop1 (float, optional): Standard deviation of the RWMH Kernel for the location parameter. Defaults to 0.1.
        std_prop2 (float, optional): Standard deviation of the RWMH Kernel for the scale parameter. Defaults to 0.1.
        std_prop3 (float, optional): Standard deviation of the RWMH Kernel for the shape parameter. Defaults to 0.1.
        List_X (bool, optional): _description_. Defaults to False.
