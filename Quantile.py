...
def Quantile_MH(Q_val, Q_sim, Q_tot, N, theta, K, I, G, distribution, std_prop):
...
def X_Q(Q_val, Q_sim, Q_tot, N, theta, K, I, G, distribution, std_prop, shape=1):
    if len(Q_sim) > 0:
        Q_sim, Q_tot = Quantile_MH(
            Q_val,
            Q_sim,
            Q_tot,
            N,
...
def Gibbs_Quantile(
    ...
    Args:
        ...
        prior_loc (str, optional): _description_. Defaults to "NIG".
        prior_scale (str, optional): _description_. Defaults to "gamma".
        prior_shape (str, optional): _description_. Defaults to "gamma".
        par_prior_loc (list, optional): _description_. Defaults to [0, 1].
        par_prior_scale (list, optional): _description_. Defaults to [0, 1].
        par_prior_shape (list, optional): _description_. Defaults to [0, 1].
        std_prop_loc (float, optional): _description_. Defaults to 0.1.
        std_prop_scale (float, optional): _description_. Defaults to 0.1.
        std_prop_shape (float, optional): _description_. Defaults to 0.1.
        std_prop_quantile (float, optional): _description_. Defaults to 0.1.
        List_X (bool, optional): _description_. Defaults to False.
        epsilon (float, optional): _description_. Defaults to 0.001.
    ...
    for i in tqdm(range(T), disable=not (verbose)):
        #print("min = {} loc = {}".format(np.min(X), loc))
        X, q_sim, q_tot = X_Q(
            Q,
            q_sim,
            q_tot,
    ...
    if verbose:
        ...
        for i in range(Q.shape[0]):
            q = Q[i]
            print(
                "Acceptance rate of Q {} = {:.2%}".format(i, (len(np.unique(q))-1) / len(q))
            )
